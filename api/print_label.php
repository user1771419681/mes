...
if ($orderId <= 0 || $articleId <= 0 || $qty <= 0) {
...
try {
    ...
    $stmtMachine = $pdo->prepare("
        SELECT pr.MachineID FROM production_order po 
    ...
    $batchCode = "B-" . date('Ymd') . "-O" . $orderId . "-" . rand(1000, 9999);
    ...
    $stmtUpdate = $pdo->prepare("
        ...
        WHERE OrderID = ? AND ArticleID = ? AND ProgressType = 'Output'
    ");
