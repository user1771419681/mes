...
def process_wago_strokes(config, window):
    ...
    try:
        ...
        if unprocessed_logs:
            ...
            for log in unprocessed_logs:
                ...
                cursor.execute("""
                    ...
                    SET EndTime = %s 
                    WHERE MachineID = %s AND EndTime IS NULL
                """, (stroke_time, machine_id))
                
                if cursor.rowcount > 0:
                    ui_log(window, f"Machine #{machine_id} resumed production. Auto-ended stop.", text_color='lightgreen')
        ...
        cursor.execute("""
            SELECT 
                ...
                TIMESTAMPDIFF(SECOND, 
                    COALESCE(
                        (SELECT MAX(Timestamp) FROM wago WHERE MachineID = pr.MachineID),
                        po.ActualStartDate,
                        NOW()
                    ), 
                    NOW()
        ...
        for am in active_machines:
            machine_id = am['MachineID']
            order_id = am['OrderID']
            inactive_seconds = am['InactiveSeconds']

            if inactive_seconds is not None and inactive_seconds > 180:
                
                cursor.execute("SELECT StopID FROM machine_stop_log WHERE MachineID = %s AND EndTime IS NULL", (machine_id,))
                if not cursor.fetchone():
                    
                    cursor.execute("SELECT OperatorID FROM operator_log WHERE MachineID = %s AND LogoutTime IS NULL ORDER BY LoginTime DESC LIMIT 1", (machine_id,))
                    op_row = cursor.fetchone()
                    operator_id = op_row['OperatorID'] if op_row else 1 

                    cursor.execute("""
                        INSERT INTO machine_stop_log (MachineID, OperatorID, ProductionOrderID, StartTime, Notes)
                        VALUES (%s, %s, %s, NOW(), 'Auto-detected stop (> 3 mins inactivity)')
                    """, (machine_id, operator_id, order_id))
                    
                    ui_log(window, f"Machine #{machine_id} inactive for {inactive_seconds}s. Auto-registered STOP.", text_color='salmon')
