...
# Define your mapping based on your classes.txt
...
def log_reject_to_db(machine_id, category_id, reason_id, window):
    ...
    try:
        ...
        if not active_order:
            # We don't spam the log here to avoid clutter during downtime
            return
...
def main():
    ...
    CONTEXT_UPDATE_INTERVAL = 5.0  # Refresh order info every 5 seconds
    ...
    while True:
        ...
        if event == '-START-':
            ...
            if not cap.isOpened():
                ...
                # Fetch initial context immediately
                fetch_production_context(machine_id, window)
        ...
        if is_running and cap:
            ...
            # Periodically poll DB for order progress updates
            ...
            # 4. Display annotated frame in GUI
